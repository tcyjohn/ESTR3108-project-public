...
from goob_ai.graph import build_agent
from goob_ai.types_new import AgentState, Message

try:
    from dotenv import load_dotenv
    load_dotenv("./.env")
    print("Loading .env file for environment variables...")

except ImportError:
    print("dotenv package not found; skipping .env loading")
    pass  # dotenv not available, continue without loading .env file
...
def run_and_submit_all( profile: gr.OAuthProfile | None) -> tuple[str, pd.DataFrame | None]:
    """Run evaluation over all questions and submit answers.

    Args:
        profile: Optional authenticated Hugging Face profile for username.

    Returns:
        A tuple of status message and a dataframe of per-question results (or None on early exit).
    """
...
def run_single_input(sentence: str) -> str:
    ...
    Args:
        sentence: The input sentence to pass to the agent.

    Returns:
        The agent's response as a string. On error returns an error message.
    """
    try:
        ...
        print(f"Error running single-input agent: {e}")
        return f"Error: {e}"
